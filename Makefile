...
build-account:
	...
	./gradlew --configuration-cache --build-cache -p services/account-service build
	@echo "Account service built successfully."

build-fit:
	@echo "Building Fit Score Service..."
	./gradlew --configuration-cache --build-cache -p services/fit-score-service build
	@echo "Fit Score service built successfully."

build-analyzer:
	@echo "Building Job Analyzer Service..."
	./gradlew --configuration-cache --build-cache -p services/job-analyzer-service build
	@echo "Job Analyzer service built successfully."

build-plan:
	@echo "Building Prep Plan Service..."
	./gradlew --configuration-cache --build-cache -p services/prep-plan-service build
	@echo "Prep Plan service built successfully."

build-profile:
	@echo "Building Profile Service..."
	./gradlew --configuration-cache --build-cache -p services/profile-service build
	@echo "Profile service built successfully."

build-auth-core:
	@echo "Building Auth Core Library..."
	./gradlew --configuration-cache --build-cache -p shared/libs/auth-core build
